...
def optimal_sequence_search(net, config, logger, train_loader, proto_feat):
    with torch.no_grad():
        net.eval()
        ...
        temp_loader = data.DataLoader(train_loader.dataset, batch_size=1, shuffle=True, num_workers=config.num_workers)
        ...
        for i in range(len(train_loader.dataset)):
            ...
            _, cas_sigmoid_fuse, _, pseudo_label, class_weighted_sum_feat, class_snippet_num, \
                x_atn, rgb_atn, flow_atn, cas_sigmoid, logits_list, labels_list = net(_data, proto_feat, None, _label)
